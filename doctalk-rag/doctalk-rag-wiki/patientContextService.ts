...
export async function buildPatientContextBlock(
  ...
  try {
    // ★ Wiki 우선 참조 (Karpathy LLM Wiki 패턴)
    // 환자별 합성 요약 문서가 있으면 facts/sources 대신 wiki 사용
    let wikiBlock = "";
    if (orgId) {
      try {
        const { getPatientWiki } = await import("./patientWikiService");
        const wiki = await getPatientWiki(patientId, orgId);
        if (wiki && wiki.content) {
          wikiBlock = wiki.content;
          console.log(`[AgentChat] Patient wiki loaded: patientId=${patientId}, tokens~${wiki.tokenCount}`);
        }
      } catch {}
    }

    const [factsBlock, sourcesBlock, conversationBlock] = await Promise.all([
      wikiBlock ? Promise.resolve("") : buildFactsBlock(patientId), // wiki 있으면 facts 스킵
      wikiBlock ? Promise.resolve("") : buildSourcesSummaryBlock(patientId), // wiki 있으면 sources 스킵
    ...
    if (wikiBlock) {
      // Wiki가 있으면 facts/sources 대신 wiki 사용
      parts.push(truncateToTokenBudget(wikiBlock, TOKEN_BUDGET.FACTS + TOKEN_BUDGET.SOURCES));
    } else {
      if (factsBlock) {
    ...
    console.log(`[AgentChat] Patient context loaded: patientId=${patientId}, tokens~${totalEstimate}${wikiBlock ? " (wiki)" : ""}`);
